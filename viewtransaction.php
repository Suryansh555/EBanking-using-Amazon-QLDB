...
<thead>
 ...
 while($rs = mysqli_fetch_array($qsql))
 {
  ...
  if($rs['from_acc_no'] != 0)
  {
	...
	<td>"  . date("d-M-Y",strtotime($rs['trans_date_time'])) . "<br>"  . date("h:i A",strtotime($rs['trans_date_time'])) . "</td>
	<td>$rs[payment_status]</td>
	<td>";
	if($rs['payment_status'] == "Pending")
	{
    echo "<a href='viewtransaction.php?trans_id=$rs[trans_id]&st=Active&acno=$rs[to_acc_no]' onclick='confirmtransaction()'  class='btn btn-primary' >Approve</a> | ";
	echo "<a href='viewtransaction.php?trans_id=$rs[trans_id]&st=Inactive&acno=$rs[to_acc_no]' onclick='confirmtransaction()' class='btn btn-danger' >Reject</a>";
	}
