...
//if($_SESSION['randomno'] == $_POST['randomno'])
{
	...
	{	
		if(isset($_GET['editid']))
			...
			$sql = "insert into transaction(to_acc_no,amount,particulars,transaction_type,trans_date_time,approve_date_time,payment_status) VALUES('$_POST[acno]','$_POST[paidamt]','Loan Amount payment of Ac No. $_POST[loanaccountno]','Debit','$dttim','$dttim','Active') ";
			$qsql = mysqli_query($con,$sql);
				
			$sql = "UPDATE accounts SET acc_balance= acc_balance -  $_POST[paidamt]  WHERE acc_no='$_POST[acno]'";
			$qsql = mysqli_query($con,$sql);
              ...
              <form role="form" id="templatemo-preferences-form" name="frmloanpayment" method="post" action="" onsubmit="return javascriptvalidation()">
                <input  type="hidden" name="randomno" value="<?php echo $_SESSION['randomno']; ?>"  />
                  ...
                  <div class="col-md-6 margin-bottom-15">
                    <label for="firstName" class="control-label">Select Loan Account Number</label>
<?php
$sqlacc ="SELECT * FROM loan WHERE status='Active' AND customer_id='$_SESSION[customer_id]'";
$qsqlacc = mysqli_query($con,$sqlacc);
?>             
<select name="loanaccountno"  class="form-control" onChange="loadloanacc(this.value)">
<option value="">Select Loan Account</option>
<?php
while($rsacc = mysqli_fetch_array($qsqlacc))
{
	echo "<option value='$rsacc[loan_acc_no]'>$rsacc[loan_acc_no]</option>";	
}
?>
</select>
                  </div>
	...
	function javascriptvalidation()     
	{
		...
		document.getElementById("jspaidamount").innerHTML ="";
		...
		*/
		...
		var validateform=0;   
			...
			{
				document.getElementById("jsloanamt").innerHTML ="<font color='red'><strong>Paid amount is not valid. Kindly enter numbers.</strong></font>";
				validateform=1;
			}	
			if(document.frmloanpayment.paidamt.value=="")
			{
				document.getElementById("jsloanamt").innerHTML ="<font color='red'><strong>Paid amount should not be empty..</strong></font>";
				validateform=1;
			}
			/*
			...
			if(!document.frmloanpayment.paidamount.value.match(numericExpression))
			{
				document.getElementById("jspaidamount").innerHTML ="<font color='red'><strong>Paid amount is not valid. Kindly enter numbers.</strong></font>";
				validateform=1;
			}	
			if(document.frmloanpayment.paidamount.value=="")
			{
				document.getElementById("jspaidamount").innerHTML ="<font color='red'><strong>Paid amount should not be empty..</strong></font>";
				validateform=1;
			}
...
function showcustomer(customeracid) 
{
        ...
        xmlhttp.open("GET","ajaxloanpayment.php?customeracid="+customeracid,true);
        xmlhttp.send();
}
</script>
 <?php
